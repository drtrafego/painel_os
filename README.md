...
| "O coletor grava a saída dentro de `web/src/`, que é código-fonte importado pelo build" (item 2 do levantamento de 10/09) | Confirmado como incidente real, medido em 20/09/2026: `servidor/servir.py` roda o coletor a cada request e escrevia por cima de `web/src/dados/estado.json`, o mesmo caminho que o git rastreia como fixture sintética do build. O commit `97656dc` (19/09, 21h39 BRT) chegou a subir uma dessas versões sujas — com dado real de operação (roster de agentes, cron, nós do Cofre, títulos de peça e valor de venda) — para o repositório **público** no GitHub. A parte do achado sobre `write_text` não ser atômico **não reproduziu**: o código já usava `tempfile.mkstemp` + `os.replace` no mesmo diretório, que é atômico em POSIX | **CONSERTADO em 20/09/2026.** `SAIDA` (coletor) e `ESTADO` (servidor) agora apontam para `data/estado.json`, fora do git (mesma pasta 700 de `aprovacoes.json`/`cofre.json`). `web/src/dados/estado.json` voltou a ser só a fixture sintética estática (conteúdo do commit `d2eab4d`, a última versão genuinamente sintética antes do vazamento), usada pelo build e pelos testes. **Pendente, fora do escopo desta correção:** o commit `97656dc` com o dado real continua no histórico do git; decidir sobre reescrever o histórico é do dono do repositório |
