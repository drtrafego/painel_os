...
export function Ferramentas({ estado, medidoEm, vista }: PropsTela) {
  ...
  const itens = filtro === 'todos' ? dados.itens : dados.itens.filter((x) => x.estado === filtro)
    ...
    const tiposMedidos = grupos.filter((grupo) => (dados.por_tipo[grupo] ?? 0) > 0 || dados.itens.some((item) => item.tipo === grupo))
    const integracoes = dados.por_tipo['Integração']
    ...
    <div className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-4"><Kpi rotulo="conexões medidas" valor={dados.medidos} nota={`${dados.por_tipo.MCP ?? 0} MCPs · ${integracoes === undefined ? 'integrações não medidas' : `${integracoes} integrações`}`} /><Kpi rotulo="disponíveis" valor={dados.contagem.disponível} cor="text-verde" /><Kpi rotulo="fallback" valor={dados.contagem.fallback} cor="text-ambar" /><Kpi rotulo="ausentes" valor={dados.contagem.ausente} cor="text-tinta-2" /></div>
    ...
    <div className="grid gap-3 lg:grid-cols-2">{tiposMedidos.length === 0 ? <div className="carta p-4 text-sm text-tinta-2">A fonte não trouxe grupos de ferramentas para exibir.</div> : tiposMedidos.map((grupo) => {
      const lista = itens.filter((x) => x.tipo === grupo)
      if (!lista.length) return null
      return <section data-grupo-ferramentas={grupo} key={grupo} className="carta p-4"><Cabecalho cor={grupo === 'MCP' ? 'var(--color-lima)' : grupo === 'App' ? 'var(--color-ciano)' : grupo === 'Integração' ? 'var(--color-ambar)' : grupo === 'Skill' ? 'var(--color-esmeralda)' : grupo === 'Acesso' ? 'var(--color-rosa)' : 'var(--color-pervinca)'} meta={`${lista.length} de ${dados.por_tipo[grupo] ?? lista.length}`}>{grupo === 'Skill' ? 'Skills Operacionais (Luana & Renato)' : grupo === 'Acesso' ? 'Conexões & Acessos de Agentes' : grupo}</Cabecalho><ul className="divide-y divide-linha">{lista.map((item) => <li data-ferramenta key={item.id} className="grid gap-2 py-3 first:pt-1 sm:grid-cols-[145px_minmax(0,1fr)_auto] sm:items-start"><div className="text-[13px] font-medium text-tinta">{item.nome}</div><div><p className="text-[11px] leading-relaxed text-tinta-2">{item.evidencia}</p><p className="mt-1 font-mono text-[9px] leading-relaxed text-tinta-3">fontes: {item.proveniencias.join(' · ')}</p>{item.fallback && <p className="mt-1 font-mono text-[9px] leading-relaxed text-ambar">via {item.fallback}</p>}</div><Pilula tom={TOM[item.estado]}>{item.estado}</Pilula></li>)}</ul></section>
    })}</div>
    <p className="mt-4 max-w-3xl text-[10px] leading-relaxed text-tinta-3">MCP é só um tipo de adaptador. APIs diretas, apps, skills e componentes locais continuam no inventário quando Claude ou Codex mudam. Disponível prova configuração local, não login eterno nem sucesso da última ação. Nenhum comando, URL privada, caminho ou credencial vai ao navegador.</p>
  </div>
}
