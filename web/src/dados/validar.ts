...
function validarVerificador(ctx: Contexto, v: unknown, p: string) {
  ...
  opcionalTexto(ctx, v.erro_leitura, `${p}.erro_leitura`); opcionalTexto(ctx, v.rodada, `${p}.rodada`)
}
...
function validarFontesEstudio(ctx: Contexto, v: unknown) {
  ...
  opcionalTexto(ctx, v.erro, 'estado.pecas.fontes_recebidas.erro'); if (!lista(ctx, v.itens, 'estado.pecas.fontes_recebidas.itens')) return
...
function validarPecas(ctx: Contexto, v: unknown) {
  ...
  opcionalTexto(ctx, v.erro, 'estado.pecas.erro'); if (!lista(ctx, v.lista, 'estado.pecas.lista')) return
  v.lista.forEach((item, i) => { const p = `estado.pecas.lista[${i}]`; if (!objetoObrigatorio(ctx, item, p)) return; numero(ctx, item.n, `${p}.n`, true, true); texto(ctx, item.titulo, `${p}.titulo`); texto(ctx, item.status, `${p}.status`); opcionalTexto(ctx, item.formato, `${p}.formato`); booleano(ctx, item.tem_capa, `${p}.tem_capa`); opcionalTexto(ctx, item.capa_url, `${p}.capa_url`); booleano(ctx, item.tem_video, `${p}.tem_video`); opcionalTexto(ctx, item.agendado_para, `${p}.agendado_para`); opcionalTexto(ctx, item.data, `${p}.data`); listaTextos(ctx, item.canais, `${p}.canais`); booleano(ctx, item.artefato_disponivel, `${p}.artefato_disponivel`); booleano(ctx, item.aprovacao_disponivel, `${p}.aprovacao_disponivel`) })
...
export function validarEstado(dado: unknown): ResultadoValidacao {
  ...
  if (objetoObrigatorio(ctx, e.verificadores, 'estado.verificadores')) for (const [nome, item] of Object.entries(e.verificadores)) { if (nome === 'erro') opcionalTexto(ctx, item, `estado.verificadores.${nome}`); else validarVerificador(ctx, item, `estado.verificadores.${nome}`) }
  if (objetoObrigatorio(ctx, e.cron, 'estado.cron')) { if (e.cron.jobs === null) { /* permitido pelo tipo */ } else if (lista(ctx, e.cron.jobs, 'estado.cron.jobs')) e.cron.jobs.forEach((j, i) => { const p = `estado.cron.jobs[${i}]`; if (objetoObrigatorio(ctx, j, p)) { texto(ctx, j.expressao, `${p}.expressao`); texto(ctx, j.rotulo, `${p}.rotulo`); opcionalTexto(ctx, j.dono, `${p}.dono`) } }); else ctx.problemas.push('estado.cron.jobs: campo ausente'); opcionalTexto(ctx, e.cron.fuso, 'estado.cron.fuso', true); opcionalNumero(ctx, e.cron.total, 'estado.cron.total', true, true, true); if (e.cron.negacao !== undefined && objetoObrigatorio(ctx, e.cron.negacao, 'estado.cron.negacao')) { opcionalTexto(ctx, e.cron.negacao.erro, 'estado.cron.negacao.erro'); opcionalNumero(ctx, e.cron.negacao.mascarados, 'estado.cron.negacao.mascarados', false, true, true) } }
